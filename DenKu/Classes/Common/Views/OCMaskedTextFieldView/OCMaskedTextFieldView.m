...
-(id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    {
//        [self configureViewShowMask:YES];
    }
    return self;
}

- (void)setMask:(NSString*)maskString showMask:(BOOL)showMask
{
    format    = maskString;
    [self configureViewShowMask:showMask];
    [self autoKeyboardDecision];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(ocMaskedTextFieldShouldBeginEditing:)]) {
        return [self.delegate ocMaskedTextFieldShouldBeginEditing:textField];
    }
    return YES;
}
...
-(void)autoKeyboardDecision
{
    ...
    [maskedTextField setKeyboardType:UIKeyboardTypePhonePad];
}
