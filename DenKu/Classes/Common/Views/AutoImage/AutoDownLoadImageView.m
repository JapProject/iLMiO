...
@interface AutoDownLoadImageView ()
                 ...
                 Default:(NSString *)defaultName
{
    ...
    if (image) {
        ...
        self.image = [manager imageWithName:defaultName];
        [manager httpForPic:path DownURL:url Json:json];
    }
}
...
{
    if (self.downURL && ![[self.downURL absoluteString] isEqualToString:[url absoluteString]]) {
        PicHttpManager *manager = GET_SINGLETON_FOR_CLASS(PicHttpManager);
...
{
    self.isClipCircle = isCircle;
    [self setImageWith:url Json:json Force:isForce SavePath:path Default:defaultName];
}
