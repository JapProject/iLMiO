...
- (void)selectMonth:(NSInteger)mon Year:(NSInteger)year
{
    NSIndexPath *path = [self pathWith:[self nameOfMonths][mon - 1] And:[NSString stringWithFormat:@"%d", year]];
    [self selectRow: path.row
        inComponent: MONTH
           animated: NO];
    
    [self selectRow: path.section
        inComponent: YEAR
           animated: NO];
}
...
{
    if (self.pickDelegate && [self.pickDelegate respondsToSelector:@selector(dateDidchanged:)]) {
        [self.pickDelegate dateDidchanged:self.date];
    }
}
...
-(NSIndexPath *)pathWith:(NSString *)month And:(NSString *)year // row - month ; section - year
{
    CGFloat row = 0.f;
    CGFloat section = 0.f;
